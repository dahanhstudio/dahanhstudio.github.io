...
Dạ Hành AG uses `@` references to create links between tasks and documentation. This allows AI assistants to automatically fetch related context.
...
```bash
dahanh task create "Add login endpoint" \
...
| Input | Resolves To |
|-------|-------------|
| `@task-42` | `.dahanh/tasks/task-42 - Title.md` |
| `@task-1` | `.dahanh/tasks/task-1 - Title.md` |

### Document References

| Input | Resolves To |
|-------|-------------|
| `@doc/auth` | `.dahanh/docs/auth.md` |
| `@doc/patterns/auth` | `.dahanh/docs/patterns/auth.md` |
| `@doc/patterns/auth.md` | `.dahanh/docs/patterns/auth.md` |
...
dahanh task create "Add caching" \
  -d "Implement Redis caching per @doc/patterns/caching"

# Less good - context buried in notes
dahanh task create "Add caching" -d "Implement caching"
# Then later...
dahanh task edit 1 --notes "See @doc/patterns/caching"
```
...
dahanh doc create "Caching Pattern" \
  -d "Redis caching strategy" \
  -f patterns

# 2. Then reference it in tasks
dahanh task create "Add user cache" \
