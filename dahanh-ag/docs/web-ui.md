...
```bash
dahanh browser --port 8080      # Custom port
dahanh browser --no-open        # Don't open browser
```
...
When you run `dahanh task edit` in CLI, the Web UI updates instantly.
...
```
Browser ←→ Express Server ←→ File System (.dahanh/)
              ↕
           SSE (real-time sync)
```

The Web UI connects to a local Express server that reads/writes to your `.dahanh/` folder. All data stays on your machine.
...
```bash
dahanh browser --port 6421
```

### Browser doesn't open

```bash
dahanh browser --no-open
