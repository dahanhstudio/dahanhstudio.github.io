...
MCP (Model Context Protocol) is a protocol that allows AI models to interact with tools via JSON-RPC. Dạ Hành AG implements an MCP server so Claude can access tasks and documentation directly.
...
const createTaskSchema = z.object({
  ...
  status: z.enum(["cần làm", "đang làm", "in-review", "hoàn thành"]).optional(),
  ...
  switch (name) {
    case "create_task": {
      ...
      const task = await fileStore.createTask({
        ...
        status: validated.status || "cần làm",
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  const path = uri.replace("dahanh://docs/", "");
...
- @doc/patterns/command-pattern - CLI Command Pattern
