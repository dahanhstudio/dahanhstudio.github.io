...
Dạ Hành AG uses file-based storage with Markdown + YAML Frontmatter instead of traditional databases. Philosophy: **"Files are the database"**.
...
class FileStore {
  ...
  async getProject(): Promise<Project | null> {
    const configPath = join(this.projectRoot, ".dahanh", "config.json");
