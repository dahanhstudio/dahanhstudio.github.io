...
Dạ Hành AG uses Server-Sent Events (SSE) for real-time synchronization between CLI, Web UI, and multiple browser tabs.
...
When you run a CLI command (e.g., `dahanh task edit 42 -s done`), the change:
...
- @doc/patterns/server-pattern - Server architecture details
- @doc/patterns/ui-pattern - React UI patterns
