...
dahanh search "keyword" --type task --plain
```
...
```bash
dahanh task create "Clear title (WHAT)" \
  -d "Description (WHY)" \
  --ac "Outcome 1" \
  --ac "Outcome 2" \
  --priority medium \
...
```bash
dahanh task create "Parent task"
dahanh task create "Subtask" --parent 48  # Raw ID only!
```
