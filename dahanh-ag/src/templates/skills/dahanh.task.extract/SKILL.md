...
**Announce at start:** "I'm using the dahanh.task.extract skill to extract knowledge from task [ID]."
...
dahanh search "<pattern/topic>" --type doc --plain

# List related docs
dahanh doc list --tag pattern --plain
```
...
```bash
dahanh doc create "Pattern: <Name>" \
...
```bash
dahanh doc edit "patterns/<name>" -c "$(cat <<'EOF'
# Pattern: <Name>
...
```bash
dahanh task edit $ARGUMENTS --append-notes "📚 Extracted to @doc/patterns/<name>"
```
