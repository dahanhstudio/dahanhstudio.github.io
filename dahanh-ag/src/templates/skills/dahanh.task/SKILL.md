...
**Announce at start:** "I'm using the dahanh.task skill to work on task [ID]."
...
dahanh task edit $ARGUMENTS -s in-progress -a @me
...
- `@task-<id>` → `dahanh task <id> --plain`
- `@doc/<path>` → `dahanh doc "<path>" --plain --smart`

```bash
# Search related docs
dahanh search "<keywords>" --type doc --plain

# Check similar completed tasks (learn from history)
dahanh search "<keywords>" --type task --status done --plain
```
...
```bash
dahanh task edit $ARGUMENTS --plan $'1. Step one (see @doc/relevant-doc)
2. Step two
3. Add tests
...
- **RELATED SKILL:** Use dahanh.task.brainstorm for complex requirements
...
dahanh task edit $ARGUMENTS --check-ac 1
dahanh task edit $ARGUMENTS --append-notes "✓ Done: brief description"
```
...
1. Stop and ask user
2. Add new AC: `dahanh task edit $ARGUMENTS --ac "New requirement"`
...
dahanh task edit $ARGUMENTS --notes $'## Summary
...
| Forget to stop timer | Always `dahanh time stop` |
