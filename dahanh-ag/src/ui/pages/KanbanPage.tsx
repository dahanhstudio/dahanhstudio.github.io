...
const BATCH_ARCHIVE_OPTIONS = [
	{ label: "1 giờ trước", value: 1 * 60 * 60 * 1000 },
	{ label: "1 ngày trước", value: 24 * 60 * 60 * 1000 },
	{ label: "1 tuần trước", value: 7 * 24 * 60 * 60 * 1000 },
	{ label: "1 tháng trước", value: 30 * 24 * 60 * 60 * 1000 },
	{ label: "3 tháng trước", value: 90 * 24 * 60 * 60 * 1000 },
];
	...
	const handleBatchArchive = async (olderThanMs: number, label: string) => {
		try {
			...
			if (result.count > 0) {
				...
				toast.success(`Đã lưu trữ ${result.count} task hoàn thành trước ${label}`);
			} else {
				toast.info(`Không có task hoàn thành trước ${label}`);
			}
		} catch (error) {
			console.error("Failed to batch archive tasks:", error);
			toast.error("Lỗi khi lưu trữ tasks");
		}
	};
		...
		<div className="flex flex-col h-full overflow-hidden">
			...
			<div className="shrink-0 p-6 pb-0">
				<div className="mb-4 flex items-center justify-between gap-4">
					<h1 className="text-2xl font-bold">Bảng Kanban</h1>
						...
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<Button variant="outline" size="default" className="gap-2">
									...
									<span className="hidden sm:inline">Lưu trữ hàng loạt</span>
								...
								{BATCH_ARCHIVE_OPTIONS.map((option) => {
									...
									return (
										<DropdownMenuItem
											...
											disabled={count === 0}
										>
											<span className="flex-1">Hoàn thành trước {option.label}</span>
