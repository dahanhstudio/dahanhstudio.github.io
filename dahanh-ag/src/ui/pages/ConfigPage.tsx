...
export default function ConfigPage() {
	...
	const handleSave = async () => {
		...
		try {
			...
			setMessage({
				type: "error",
				text: err instanceof Error ? err.message : "Không thể lưu cấu hình",
			});
		} finally {
			setSaving(false);
		}
	...
	const handleAddStatus = () => {
		...
		if (currentStatuses.includes(statusKey)) {
			setMessage({ type: "error", text: "Trạng thái này đã tồn tại" });
			return;
		}
		setConfig({
			...config,
	...
	if (loading) {
		return (
			<div className="p-6 flex items-center justify-center h-64">
				<div className="text-lg text-muted-foreground">Đang tải cấu hình...</div>
			</div>
		);
	}
			...
			{viewMode === "json" ? (
				...
				<div className="bg-card rounded-lg border p-6 space-y-4">
					<div>
						...
						<textarea
							...
							spellCheck={false}
						/>
						{jsonError && <p className="mt-2 text-sm text-destructive">Lỗi cú pháp JSON</p>}
					</div>
				...
				<div className="bg-card rounded-lg border p-6 space-y-6">
					...
					<div>
						...
						/>
						<p className="mt-1 text-xs text-muted-foreground">Tên hiển thị của dự án này</p>
					</div>

					{/* Default Assignee */}
					<div>
						<label className="block text-sm font-medium mb-2">Người thực hiện mặc định</label>
						<Input
							type="text"
						...
						/>
						<p className="mt-1 text-xs text-muted-foreground">Người được gán mặc định cho task mới (ví dụ: @antigravity)</p>
					</div>

					{/* Default Priority */}
					<div>
						<label className="block text-sm font-medium mb-2">Độ ưu tiên mặc định</label>
						<select
						...
						<p className="mt-1 text-xs text-muted-foreground">Độ ưu tiên mặc định khi tạo task mới</p>
					</div>

					{/* Default Labels */}
					<div>
						<label className="block text-sm font-medium mb-2">Nhãn mặc định</label>
						<Input
							type="text"
						...
						/>
						<p className="mt-1 text-xs text-muted-foreground">Danh sách nhãn mặc định (phân cách bằng dấu phẩy)</p>
					</div>

					{/* Time Format */}
					<div>
						<label className="block text-sm font-medium mb-2">Định dạng thời gian</label>
						<select
						...
						<p className="mt-1 text-xs text-muted-foreground">Cách hiển thị thời gian trong ứng dụng</p>
					</div>

					{/* Editor */}
					<div>
						<label className="block text-sm font-medium mb-2">Trình soạn thảo mặc định</label>
						<Input
							type="text"
							value={config.editor || ""}
						...
						/>
						<p className="mt-1 text-xs text-muted-foreground">Lệnh mở trình soạn thảo để sửa file</p>
					</div>

 					{/* Statuses */}
 					<div>
						<label className="block text-sm font-medium mb-2">Trạng thái công việc</label>
						...
						<p className="mt-2 text-xs text-muted-foreground">
							Tùy chỉnh các trạng thái công việc và màu sắc hiển thị trên bảng Kanban
						</p>
					</div>

					{/* Visible Kanban Columns */}
					<div>
						<label className="block text-sm font-medium mb-2">Cột Kanban hiển thị</label>
						...
						<p className="mt-2 text-xs text-muted-foreground">Chọn các cột sẽ hiển thị trên bảng Kanban</p>
					</div>
