...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
	...
	const docsDir = join(process.cwd(), ".dahanh", "docs");

	// Task resources
	const taskResources = tasks.map((task) => ({
		uri: `dahanh://task/${task.id}`,
		name: task.title,
	...
	if (existsSync(docsDir)) {
		...
		for (const file of mdFiles) {
			...
			docResources.push({
				uri: `dahanh://doc/${file.replace(/\.md$/, "")}`,
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
	...
	const taskMatch = uri.match(/^dahanh:\/\/task\/(.+)$/);
	if (taskMatch) {
	...
	const docMatch = uri.match(/^dahanh:\/\/doc\/(.+)$/);
	if (docMatch) {
