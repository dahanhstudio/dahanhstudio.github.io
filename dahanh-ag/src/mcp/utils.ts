...
export async function fetchLinkedDocs(task: Task): Promise<Array<{ path: string; title: string; content: string }>> {
	...
	const docsDir = join(projectRoot, ".dahanh", "docs");
	...
	// Resolves documentation references (@doc/name) using .dahanh/docs content
	...
	for (const ref of docRefs) {
		...
		try {
			// Extract filename from resolved path (@.dahanh/docs/filename.md)
			const filename = ref.resolvedPath.replace("@.dahanh/docs/", "");
