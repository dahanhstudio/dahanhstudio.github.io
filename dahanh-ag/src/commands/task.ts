...
async function formatTask(task: Task, fileStore: FileStore, plain = false): Promise<string> {
	if (plain) {
		...
		if (task.labels.length > 0) {
			output.push(`Nhãn: ${task.labels.join(", ")}`);
		}
		...
		if (task.timeSpent > 0) {
			...
			output.push(`Thời gian đã dùng: ${hours}h ${minutes}m`);
		}
		...
		if (task.acceptanceCriteria.length > 0) {
			output.push("Tiêu chí Chấp nhận:");
			output.push(border);
		...
		if (task.implementationPlan) {
			output.push("Kế hoạch Thực hiện:");
			output.push(border);
	...
	output.push(`${chalk.gray("Trạng thái:")} ${statusColor(task.status)}`);
	output.push(`${chalk.gray("Độ ưu tiên:")} ${priorityColor(task.priority)}`);

	if (task.assignee) {
		output.push(`${chalk.gray("Người thực hiện:")} ${chalk.cyan(task.assignee)}`);
	}

	if (task.labels.length > 0) {
		output.push(`${chalk.gray("Nhãn:")} ${task.labels.map((l) => chalk.blue(l)).join(", ")}`);
	}

	if (task.parent) {
		output.push(`${chalk.gray("Task cha:")} ${chalk.yellow(task.parent)}`);
	}
	...
	// Subtasks
	if (task.subtasks.length > 0) {
		output.push(chalk.bold("Task con:"));
	...
	// Acceptance Criteria
	if (task.acceptanceCriteria.length > 0) {
		output.push(chalk.bold("Tiêu chí Chấp nhận:"));
		for (const [i, ac] of task.acceptanceCriteria.entries()) {
	...
	// Implementation Plan
	if (task.implementationPlan) {
		output.push(chalk.bold("Kế hoạch Thực hiện:"));
		output.push(task.implementationPlan);
		output.push("");
	}
	...
	// Timestamps
	output.push(chalk.gray(`Đã tạo: ${task.createdAt.toISOString()}`));
	output.push(chalk.gray(`Cập nhật: ${task.updatedAt.toISOString()}`));
